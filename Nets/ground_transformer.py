"""
The code is from:
https://github.com/dongzhang89/FPT
...
class GroundTrans(nn.Module):
    def __init__(self, in_channels, inter_channels=None, mode='dot', dimension=1, bn_layer=True):
    ...
    def forward(self, x_low, x_high):
        ...
        """
        batch_size = x_low.size(0)  # 降维减少计算量
        ...
        y = y.view(batch_size, self.inter_channels, x_low.size()[2], x_low.size()[2])
...
if __name__ == '__main__':
    x_1 = torch.rand([2, 256, 14, 14]).cuda()
    x_2 = torch.rand([2, 256, 7, 7]).cuda()
    gt = GroundTrans(in_channels=256, dimension=2).cuda()
    c = gt(x_1, x_2)
    print(c.shape)
    # a = euclidean([[1,2],[2,3])
    # print(a)
